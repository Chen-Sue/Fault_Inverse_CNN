...
# file_location = r"C:\chengshu\ShiYaolin\Program\data_create\10w_mean"  # location可修改
...
class MergeData:
    ...
    def pkl_dump_x(self, x):
        inputdata = open(file_location + r"/inputdata3.pkl", "wb")
        pickle.dump(x, inputdata)
        inputdata.close()         

    def pkl_dump_y(self, y):
        outputdata = open(file_location + r"/outputdata3.pkl", "wb")
    ...
    def mergeInputFile(self): 
        ...
        x = pd.DataFrame() 
        t0 = time.time()
        for i in range(int(numbers/10)*4+1, int(numbers/10)*6+ 1):
            inputData = pd.read_csv(file_location + r"/disp_mean_csv" + r"/%d"%i + fileFrom, \
                                   delimiter=",", \
                                   usecols=[0,1,2,3,4],\
                                   header=None)
    ...
    def mergeOutputFile(self): 
        ...
        for i in range(1, numbers+1): 
            ...
            outputData = pd.read_csv(file_location + r"/inp_mean_csv" + r"/%d"%i + fileFrom, \
                                     delimiter=",", \
    ...
    def pkl_load_x(self):
        ...
        inputdata = open(file_location + r"/inputdata3.pkl", "rb")
    ...
    def pkl_load_y(self):
        outputdata = open(file_location + r"/outputdata3.pkl", "rb")
