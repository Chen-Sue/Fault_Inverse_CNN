...
def numberFile():
    ...
    number = 0
    file_glob = os.path.join(file_location, r"/10w_mean/inp_mean")
...
def main():
    for i in range(1, numberFile()+1):   
        with open(file_location + r"/edcmpdata_bam/edcmp_bam.inp", mode="r", encoding="utf-8") as fin, \
                open(file_location + r"/10w_mean/inp_mean" + r"/%d"%i + fileFrom, mode="w+", encoding="utf-8") as fout:
            for line in fin:     
                if "izmhs" in line:  
                    # modify  file_name
