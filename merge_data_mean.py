...
print("\n  Begin merge_data_mean.py ... \n")

t = time.time()

# fileLocation = r"C:\chengshu\ShiYaolin\Program\data_create\10w_mean"  # location可修改
...
def numberFile():
    ...
    number = 0
    file_glob = os.path.join(file_location, r"/disp_mean_csv")
    for _ in os.listdir(file_glob):
        number += 1
    return number

numbers = numberFile()
print("\n  number of files: ", numberFile())
...
class MergeData:
    ...
    def pkl_dump_x(self, x):
        inputdata = open(file_location + r"/inputdata_mean1.pkl", "wb")
        pickle.dump(x, inputdata)
        inputdata.close()         

    def pkl_dump_y(self, y):
        outputdata = open(file_location + r"/outputdata_mean1.pkl", "wb")
    ...
    def mergeInputFile(self): 
        ...
        for i in range(1, numbers+1):
            inputData = pd.read_csv(fileLocation + r"/disp_mean_csv" + r"/%d"%i + fileFrom, \
                                   delimiter=",", \
                                   usecols=[0,1,2,3,4],\
                                   header=None)
    ...
    def mergeOutputFile(self): 
        ...
        t0 = time.time()
        for i in range(1, numbers+1):  
            outputData = pd.read_csv(fileLocation + r"/inp_mean_csv" + r"/%d"%i + fileFrom, \
                                     delimiter=",", \
    ...
    def pkl_load_x(self):        
        inputdata = open(file_location + r"/inputdata_mean1.pkl", "rb")
    ...
    def pkl_load_y(self):
        outputdata = open(file_location + r"/outputdata_mean1.pkl", "rb")
