...
class ResnetBlock(keras.Model):
    def __init__(self, channels, strides=1, residual_path=False):
        ...
        self.bn1 = keras.layers.BatchNormalization(beta_regularizer=regularizers.l2(0.1))
        self.conv2 = conv3x3(channels)
        self.bn2 = keras.layers.BatchNormalization(beta_regularizer=regularizers.l2(0.1))
        if residual_path:
            self.down_conv = conv3x3(channels, strides, kernel=(1, 1))
            self.down_bn = keras.layers.BatchNormalization(beta_regularizer=regularizers.l2(0.1))
    def call(self, inputs, training=None):
        ...
        x = tf.nn.relu(x)
        x = self.conv2(x)
        # this module can be added into self. however, module in for can not be added.
...
class ResNet(keras.Model):
    def __init__(self, block_list, output_node, initial_filters=16, **kwargs):
        ...
        for block_id in range(len(block_list)):
            ...
            self.out_channels *= 2
        self.final_bn = keras.layers.BatchNormalization(beta_regularizer=regularizers.l2(0.1))
        self.avg_pool = keras.layers.GlobalAveragePooling2D()
        # self.avg_pool = keras.layers.GlobalMaxPool2D()
        self.fc = keras.layers.Dense(output_node)
    def call(self, inputs, training=None):
...
def main():
    ...
    # model = ResNet([3, 4, 6, 3], output_node) # resnet34
    model = ResNet([2, 2, 2, 2], output_node) # resnet18
